...
{
  struct addrinfo *address = user_data;
  int socket_fd = 0;
...

  {
    ...
    for (i = 0; i < n_messages; i++) {
      ...
      written_size = write(socket_fd, message, sizeof(message) - 1);
      if (written_size == -1) {
...
main(int argc, char **argv)
{
...

  {
    ...
    pool = g_thread_pool_new(worker, &(addresses[0]), n_workers, exclusive, &error);
    if (error) {
